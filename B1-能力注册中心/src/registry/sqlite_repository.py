...
from .repository import DefinitionFilter
...
class SQLiteDefinitionRepository(InMemoryDefinitionRepository):
    ...
    def _load(self):
        ...
        for row in self._db.execute("SELECT * FROM definition_versions ORDER BY rowid"):
            self._versions.setdefault(row["object_id"], []).append({
                "object_id": row["object_id"], "version": row["version"],
                "revision": row["revision"], "content_digest": row["content_digest"],
                "spec_snapshot": json.loads(row["spec_snapshot"]),
                "created_at": row["created_at"], "created_by": json.loads(row["created_by"]),
            })
    ...
    async def create(self, obj: dict) -> str:
        ...
        try:
            ...
            self._db.execute(
                ...
                (result, meta["namespace"], meta["key"], meta["version"],
                 meta.get("revision", 1), obj["kind"], self._j(obj)),
            )
    ...
    async def save_version(self, object_id, version, revision, content_digest, spec_snapshot, created_by):
        await super().save_version(object_id, version, revision, content_digest, spec_snapshot, created_by)
        item = self._versions[object_id][-1]
        self._db.execute("INSERT INTO definition_versions VALUES (?,?,?,?,?,?,?)", (
            object_id, version, revision, content_digest, self._j(spec_snapshot),
            item["created_at"], self._j(created_by)))
        self._commit_if_needed()

    async def save_dependency(self, source_id, source_version, target_id, target_version, dependency_type):
        before = len(self._dependencies)
        await super().save_dependency(source_id, source_version, target_id, target_version, dependency_type)
        if len(self._dependencies) > before:
            self._db.execute("INSERT OR IGNORE INTO definition_dependencies VALUES (?,?,?,?,?)",
                             (source_id, source_version, target_id, target_version, dependency_type))
            self._commit_if_needed()

    async def save_audit_log(self, action, resource_id, resource_kind, actor_type, actor_id,
                             tenant_id=None, before_state=None, after_state=None, details=None):
        await super().save_audit_log(action, resource_id, resource_kind, actor_type, actor_id,
                                     tenant_id, before_state, after_state, details)
        item = self._audit_logs[-1]
        self._db.execute("INSERT INTO audit_logs(action,resource_id,resource_kind,actor_type,actor_id,tenant_id,before_state,after_state,details,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
                         (action, resource_id, resource_kind, actor_type, actor_id, tenant_id,
                          ...
                          self._j(details or {}), item["created_at"]))
        self._commit_if_needed()
