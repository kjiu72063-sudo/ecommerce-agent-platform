...
import path_config  # noqa: F401 - 自动设置 B0 路径
...
class RegistryService:
    ...
    async def transition_state(self, id: str, target_phase: str, 
                               reason: str = None, actor: dict = None) -> dict:
        """迁移定义对象状态
    ...
    async def delete_definition(self, id: str, actor: dict) -> bool:
        ...
        if phase != 'draft':
            raise ValueError(f"Cannot delete object in '{phase}' state. Only 'draft' objects can be deleted.")
    ...
    async def list_definitions(self, filter: DefinitionFilter, 
    ...
    async def resolve_definition(self, id: str, version: str, purpose: str = "new_run") -> dict:
        ...
        if obj['metadata']['version'] != version:
            raise ValueError(f"Version mismatch: expected {version}, got {obj['metadata']['version']}")
    ...
    async def resolve_dependency_graph(self, id: str, version: str, purpose: str = "new_run") -> dict:
        """递归解析以 (id, version) 为根的依赖图。"""
        from .dependency_resolver import DependencyResolver
        return await DependencyResolver(self.repo).resolve(id, version, purpose)

    async def _extract_and_save_dependencies(self, source_id: str, source_version: str,
                                              spec: dict, kind: str) -> None:
        ...
        for ref_list_name, dep_type in [
            ...
            for ref in ref_list:
                ...
                if actual_ref and 'id' in actual_ref:
                    await self._save_dependency_checked(source_id, source_version, actual_ref, dep_type)

    async def _save_dependency_checked(self, source_id: str, source_version: str,
                                       ref: dict, dependency_type: str) -> None:
