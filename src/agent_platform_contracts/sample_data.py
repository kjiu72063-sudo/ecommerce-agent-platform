...
def base(
    kind: str, spec: dict[str, Any], status: dict[str, Any], definition: bool
) -> dict[str, Any]:
    key_map = {
...
def valid_examples() -> dict[str, dict[str, Any]]:
    agent_spec = base(
        "AgentSpec",
        {
            ...
            "skill_bindings": [
                {
                    "selector": {"domain": "development", "status": "active", "labels": {}},
                    "max_candidates": 8,
                }
            ],
            "tool_bindings": [
    ...
    skill = base(
        "SkillManifest",
        {
            ...
            "description": (
                "Inspect an authorized repository and summarize its structure "
            ...
            "required_tools": [
                {"tool_ref": ref("ToolManifest"), "required_capabilities": ["repository.read"]}
            ],
            "required_skills": [],
            "conflicts_with": [],
            "permission_requirements": [
                {
                    ...
                    "resources": ["project:current/repository:*"],
                    "conditions": {"environment": "sandbox"},
                }
            ],
            "risk_level": "low",
    ...
    tool = base(
        "ToolManifest",
        {
            ...
            "idempotency": {
                "supported": False,
                "key_scope": "none",
                "duplicate_semantics": "not_applicable",
            },
            ...
            "health_contract": {
                "healthcheck_key": "repository-gateway/health",
                "interval_seconds": 30,
            ...
            "fragments": [
                {
                    ...
                    "layer": "safety",
                    "order": 0,
                    "content": "Respect permissions, approvals, and task scope.",
                },
                {
                    "name": "programming-identity",
                    "layer": "identity",
                    "order": 10,
                    "content": "You are a bounded programming agent.",
                },
                {
                    "name": "structured-output",
                    "layer": "output_contract",
                    "order": 20,
                    "content": "Return an object matching the declared output schema.",
                },
            ],
    ...
    model = base(
        "ModelPolicy",
        {
            ...
            "fallback_chain": [
                {
                    "route_id": "fallback-model",
                    "on_error_codes": ["MODEL_UNAVAILABLE", "RATE_LIMITED"],
                }
            ],
        ...
        "ContextPolicy",
        {
            "source_rules": [
                {"source": "task", "effect": "allow", "priority": 100, "filters": {}},
                {
                    ...
                    "effect": "allow",
                    "priority": 60,
                    "filters": {"scope": ["project", "user"]},
                },
                {
                    "source": "knowledge",
                    "effect": "allow",
                    "priority": 70,
                    "filters": {"require_provenance": True},
                },
            ],
            "token_budget": {
                "total_tokens": 50000,
            ...
            "freshness": {
                ...
                "source_ttl_seconds": {"tool_result": 300, "knowledge": 86400},
            },
            ...
            "redaction": {
                "redact_secrets": True,
                "pii_mode": "mask",
                "restricted_data_mode": "local_only",
            },
            ...
            "rules": [
                {
                    ...
                    "actions": ["repository.read"],
                    "resources": ["project:current/repository:*"],
                    "conditions": {"environment": ["development", "sandbox"]},
                },
                {
    ...
    task = base(
        "Task",
        {
            ...
            "source": {
                "type": "openclaw",
                "channel": "feishu",
                "source_message_id": "message-12345",
            },
            ...
            "routing_constraints": {
                "allowed_agent_refs": [ref("AgentSpec")],
            ...
            "dynamic_dependency_activations": [
                {
                    ...
                    "activation_reason": (
                        "Selected from the permission-filtered Skill catalog for "
    ...
    checkpoint = base(
        "Checkpoint",
        {
            ...
            "resume_token_ref": {
                "provider": "vault",
                "key": "runtime/checkpoints/run-001",
                "version": 1,
                "field": "resume_token",
            },
    ...
    approval = base(
        "Approval",
        {
            ...
            "requested_actions": [
                {"action": "repository.write", "resource": "project:current/repository:repo_demo"}
            ],
            "input_digest": f"sha256:{214:064x}",
            "policy_snapshot": ref("PermissionProfile"),
            "required_approvers": [
                {"role": "project-owner", "count": 1, "separation_of_duties": False}
            ],
            "decisions": [
                {
                    "approver": actor(),
                    "decision": "approve",
                    "decided_at": LATER,
                    "comment": "Approved for development branch.",
                }
            ],
            "expires_at": EXPIRES,
            "one_time": True,
    ...
    artifact = base(
        "Artifact",
        {
            ...
            "locator": {
                "provider": "s3",
                "bucket_or_repository": "agent-artifacts",
                "object_key": "tenants/default/projects/demo/results/report.md",
                "version_id": "v1",
            },
...
def invalid_examples() -> dict[str, dict[str, Any]]:
    ...
    add(
        ...
        lambda p: p.update({"undeclared": True}),
    )
    add(
        "skill-hermes-active",
        "skill-manifest",
        "HERMES_PROPOSAL_NOT_ACTIVATABLE",
        lambda p: p["spec"]["provenance"].update({"type": "hermes_proposal"}),
    )
    ...
    add(
        "tool-side-effect-without-idempotency",
        "tool-manifest",
    ...
    add(
        ...
        "loop-profile",
        "LOOP_HARD_LIMIT_REQUIRED",
        lambda p: p["spec"].update({"max_iterations": 0}),
    )
    add(
        "permission-default-allow",
        "permission-profile",
        "DEFAULT_DENY_REQUIRED",
        lambda p: p["spec"].update({"default_decision": "allow"}),
    )
    add(
        "task-missing-idempotency-key",
        "task",
        "TASK_IDEMPOTENCY_REQUIRED",
        lambda p: p["spec"].pop("idempotency_key"),
    )
    add(
        "run-latest-dependency",
        "agent-run",
        "EXACT_VERSION_REQUIRED",
        lambda p: p["spec"]["resolved_dependencies"]["prompt_package_ref"].update(
            {"version": "latest"}
        ),
    )
    ...
    add(
        "run-dynamic-activation-sequence-gap",
        "agent-run",
        "DYNAMIC_ACTIVATION_SEQUENCE_INVALID",
        invalid_dynamic_sequence,
    )
    ...
    add(
        "run-dynamic-activation-duplicate",
        "agent-run",
        "DYNAMIC_DEPENDENCY_DUPLICATE",
        invalid_dynamic_duplicate,
    )
    ...
    add(
        "run-model-current-mismatch",
        "agent-run",
        "MODEL_SELECTION_CURRENT_MISMATCH",
        invalid_model_current,
    )
    ...
    add(
        "run-model-fallback-missing-trigger",
        "agent-run",
        "MODEL_FALLBACK_TRIGGER_REQUIRED",
        invalid_model_fallback_trigger,
    )
    add(
        "checkpoint-zero-sequence",
        "checkpoint",
        "CHECKPOINT_SEQUENCE_INVALID",
        lambda p: p["spec"].update({"sequence": 0}),
    )
    ...
    add(
        "tool-call-side-effect-without-key",
        "tool-call",
    ...
    add(
        "approval-one-time-used-twice",
        "approval",
        "APPROVAL_ALREADY_CONSUMED",
        lambda p: p["spec"].update({"usage_count": 2}),
    )
    add(
        "artifact-signed-locator",
        "artifact",
        "ARTIFACT_LOCATOR_SECRET",
        lambda p: p["spec"]["locator"].update({"object_key": "report.md?X-Amz-Signature=secret"}),
    )
    add(
        "event-zero-sequence",
        "event",
