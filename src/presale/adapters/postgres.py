...
class PostgresProductQuestionRepository(ProductQuestionRepository):
    ...
    async def find_by_idempotency(
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT content FROM presale_questions WHERE tenant_id = $1 AND idempotency_key = $2",
                tenant_id,
                idempotency_key,
            )
...
class PostgresEvidenceRepository(EvidenceRepository):
    ...
    async def save_evidence(self, run_ref: str, items: list[EvidenceItem]) -> None:
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                "DELETE FROM presale_evidence WHERE run_ref = $1", run_ref
            )
            for item in items:
                await conn.execute(
                    "INSERT INTO presale_evidence (run_ref, tenant_id, content) VALUES ($1, $2, $3::jsonb)",
                    run_ref,
                    item.tenant_id,
                    _json_dumps(item),
                )
...
class PostgresAnswerDraftRepository(AnswerDraftRepository):
    ...
    async def save(self, draft: AnswerDraft, *, tenant_id: str) -> None:
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO presale_answers (run_ref, tenant_id, content) VALUES ($1, $2, $3::jsonb) "
...
class PostgresIdempotencyRepository(IdempotencyRepository):
    ...
    async def claim(self, record: IdempotencyRecord) -> IdempotencyRecord:
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT content FROM presale_idempotency WHERE tenant_id = $1 AND idempotency_key = $2",
                record.tenant_id,
...
class PostgresRunTraceRepository(RunTraceRepository):
    ...
    async def save(self, trace: PresaleRunTrace) -> None:
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO presale_traces (run_ref, tenant_id, content) VALUES ($1, $2, $3::jsonb) "
                "ON CONFLICT (run_ref) DO UPDATE SET content = EXCLUDED.content",
                trace.run_ref,
                trace.tenant_id,
                _json_dumps(trace),
