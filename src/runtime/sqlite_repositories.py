    ...
    async def create_task(self, task: dict) -> str:
        ...
        try:
            self.store.db.execute(
                ...
                (result, json.dumps(task, ensure_ascii=False, sort_keys=True)),
            )
            self.store.commit_if_needed()
        except Exception:
            self.store.db.rollback()
            self._tasks.pop(result, None)
            raise
        return result
    ...
    async def update_task_status(self, task_id, phase, updates=None):
        ...
        if ok:
            self.store.db.execute(
                ...
                (json.dumps(self._tasks[task_id], ensure_ascii=False, sort_keys=True), task_id),
            )
            self.store.commit_if_needed()
        return ok
...
class SQLiteRunRepository(InMemoryRunRepository):
    def __init__(self, database: str | Path = "b2_runtime.sqlite3", store=None):
        ...
        for row in self.store.db.execute("SELECT content FROM runs"):
            ...
            self._runs[run["metadata"]["id"]] = run
    ...
    async def create_run(self, run: dict) -> str:
        ...
        try:
            self.store.db.execute(
                ...
                (result, json.dumps(run, ensure_ascii=False, sort_keys=True)),
            )
            self.store.commit_if_needed()
        except Exception:
            self.store.db.rollback()
            self._runs.pop(result, None)
            raise
        return result
    ...
    async def update_run_status(self, run_id, phase, updates=None):
        ...
        if ok:
            self.store.db.execute(
                "UPDATE runs SET content=? WHERE id=?",
                (json.dumps(self._runs[run_id], ensure_ascii=False, sort_keys=True), run_id),
            )
            self.store.commit_if_needed()
        return ok
    ...
    async def save_version(self, run_id: str, spec_snapshot: dict) -> None:
        ...
        self.store.db.execute(
            "INSERT INTO run_versions VALUES (?,?,?)",
            (
                run_id,
                json.dumps(spec_snapshot, ensure_ascii=False, sort_keys=True),
                item["created_at"],
            ),
        )
        self.store.commit_if_needed()
    ...
    async def get_versions(self, run_id: str) -> list[dict]:
        rows = self.store.db.execute(
            "SELECT spec_snapshot, created_at FROM run_versions WHERE run_id=? ORDER BY rowid",
            (run_id,),
        ).fetchall()
        return [
            {
...
class SQLiteCheckpointRepository(InMemoryCheckpointRepository):
    def __init__(self, database: str | Path = "b2_runtime.sqlite3", store=None):
        ...
        for row in self.store.db.execute(
            "SELECT content FROM checkpoints ORDER BY run_id, sequence"
        ):
...
class SQLiteEventRepository(InMemoryEventRepository):
    def __init__(self, database: str | Path = "b2_runtime.sqlite3", store=None):
        ...
        self.store = store or SQLiteRuntimeStore(database)
        for row in self.store.db.execute(
            "SELECT content FROM events ORDER BY subject_id, sequence"
        ):
    ...
    async def save_event(self, event: dict) -> str:
        ...
        self.store.db.execute(
            ...
            (event_id, subject_id, sequence, json.dumps(event, ensure_ascii=False, sort_keys=True)),
        )
        self.store.commit_if_needed()
    ...
    async def get_next_sequence(self, subject_id: str) -> int:
        row = self.store.db.execute(
            "SELECT COALESCE(MAX(sequence),0)+1 FROM events WHERE subject_id=?", (subject_id,)
        ).fetchone()
        return int(row[0])
