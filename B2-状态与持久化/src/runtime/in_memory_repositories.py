...
from .repositories import TaskRepository, RunRepository, EventRepository, CheckpointRepository, TaskFilter
...
class InMemoryCheckpointRepository(CheckpointRepository):
    ...
    async def list_checkpoints(self, run_id: str) -> list[dict]:
        results = [
            copy.deepcopy(cp) for cp in self._checkpoints.values()
