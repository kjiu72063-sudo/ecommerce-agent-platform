...
from runtime import TaskService, EventStore, InMemoryTaskRepository, InMemoryEventRepository, TaskFilter
...
async def create_task(request: CreateTaskRequest):
    ...
    try:
        ...
        result = await service.create_task(request.payload, {"actor_type": request.actor_type, "actor_id": request.actor_id})
...
async def validate_task(task_id: str, request: TaskActionRequest):
    ...
    try:
        service = get_task_service()
        result = await service.validate_task(task_id, {"actor_type": request.actor_type, "actor_id": request.actor_id})
        return ApiResponse(status="success", data=result)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def fail_task(task_id: str, error_code: str = "INTERNAL_ERROR", error_message: str = "Unknown error"):
    """任务失败"""
    try:
        service = get_task_service()
