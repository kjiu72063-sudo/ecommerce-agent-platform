...
- **当前最大风险（历史，已缓解）**：指南提出的 3 个 Agent（售前/直播切片/自媒体）需要 ASR、视频处理、图像生成等外部服务，超出"单进程本地闭环"边界。直播切片/自媒体已按 mock 外部服务落地（PR #124，方向5），真实外部服务接入仍是后续项
...
- **做什么**：
  ...
  4. 更新路线图和交付物清单
- **不做什么**：
  - ~~不实现直播切片/自媒体 Agent~~ **（范围已更新）**：方向5 已交付 `LiveClipperAgent` / `ContentCreatorAgent`（PR #124），外部 ASR/图像服务为 **mock**、接口与真实同形；**不接入真实外部服务**仍是边界
  - 不引入真实的 ASR、视频处理、图像生成服务（mock 可替换端口已就位）
  - 不实现复杂 Skill 触发机制
  - 不实现 AgentSpec JSON Schema 配置加载（当前 PresaleAgent 是硬编码的）
  - 不接入真实 LLM（确定性/模拟；真实 LLM 验收走独立 `workflow_dispatch` job）
- **不承诺**：
  - 不承诺 mock Agent 换真实外部服务的时间表
