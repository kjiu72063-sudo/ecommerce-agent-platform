...
class TestContextService(unittest.TestCase):
    def setUp(self):
        self.policy = json.loads((path_config.B0_EXAMPLES / "valid" / "context-policy.json").read_text(encoding="utf-8"))
    ...
    def _descriptor(self, extra_sources=None):
        return {
            ...
            "policy_ref": {"kind": "ContextPolicy", "id": "cpo_0198f6d0-7ef0-7b0e-a0d3-5f9c96c7f416",
                           "version": "1.0.0", "digest": canonical_sha256({"p": 1})},
            "artifact_ref": {"id": "art_0198f6d0-7ef0-7b0e-a0d3-5f9c96c7f427",
                             "digest": canonical_sha256({"a": 1})},
            "redaction_summary": {"secret_count": 0, "pii_count": 0},
            "sources": [
                {"type": "task", "content": "用户咨询", "priority": 100, "provenance": ["tsk_0198f6d0-7ef0-7b0e-a0d3-5f9c96c7f421"]},
                {"type": "agent_spec", "content": "售前咨询 Agent 定义", "priority": 90, "provenance": ["agt_0198f6d0-7ef0-7b0e-a0d3-5f9c96c7f411"]},
                {"type": "prompt", "content": "你是一个专业的电商售前客服", "priority": 80, "provenance": ["prm_0198f6d0-7ef0-7b0e-a0d3-5f9c96c7f414"]},
            ] + (extra_sources or []),
        }
    ...
    def test_deduplication(self):
        service = ContextService(self.policy)
        descriptor = self._descriptor(extra_sources=[
            {"type": "task", "content": "用户咨询", "priority": 50, "provenance": ["tsk_0198f6d0-7ef0-7b0e-a0d3-5f9c96c7f422"]}
        ])
