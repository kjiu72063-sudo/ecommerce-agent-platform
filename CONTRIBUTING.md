...
- **检索回归**：Milvus job 用确定性嵌入跑 `presale-eval --mode retrieval`，再 `--compare-batch tests/fixtures/retrieval_golden_snapshot.json::<current> --fail-on-regression --rank-tolerance 2` 对照快照。带内名次抖动（Δ≤2）不计回退（吸收 ANN/RRF 跨 run 噪声）；召回丢失（rank→None）仍必失败。回退时失败并上传 `eval-artifacts/`。
