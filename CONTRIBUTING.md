...
- **门禁纪律**：CI 为 `main` 与每个 PR 运行上述前三条。注意：当前仓库是 GitHub **免费私有仓，无法启用 branch protection / required checks**，因此 CI 对 PR 无法在合并时硬性拦截，是"合并前须人工核对通过"的纪律而非平台强制。合并前请先确认该 PR 的 CI `pass`（`gh pr checks`），若仍 `pending/queued` 应等它跑完；**未通过不得合并**。
- **已知例外**：2026-09-13 PR #9/#10 曾因免费层 runner 排队过久、在 CI 尚未跑完时被合并（事后 CI 均通过）。这是违反门禁纪律的既有记录，后续不得再以排队为由跳过等待。
