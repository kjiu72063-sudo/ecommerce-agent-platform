...
### Added
- **Curated demo answers**: `presale/data/demo_curated.json` holds 4 pre-approved archive answers (captured from real LLM runs, all finalize+supported); exact trimmed question+product match on the stream endpoint streams the archive (retrieval/evidence/timeline stay live) with `curated: true`, UI shows a「演示精选 · 预审存档」badge. Guards presentation quality against live-LLM variance. 3 tests. Baseline 433.
