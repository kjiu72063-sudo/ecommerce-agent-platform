...
| `eval_suite_refs` | ResourceRef[] | 是 | 上线前评估集 |
...
一个 Task 可以因为重试、人工恢复、模型切换或 Agent 切换产生多个 AgentRun，但任一时刻必须遵守其并发策略。
...
| `resolved_dependencies` | Prompt、Skill、Tool、Policy、Loop、模型的精确快照 |
...
- 运行中发生模型回退、Skill 激活和权限变化必须产生 Event。
...
运行中按需激活的 Skill 和 Tool 必须追加到快照记录，不能覆盖历史。
...
10. 文档版本升级为 `0.2.0` 或 `1.0.0` 并进入 `approved`。

---

# B0 后续未决项

以下问题不阻塞本方案评审，但必须在进入实现前确认：

| ID | 未决项 | 建议默认值 |
|---|---|---|
| O-01 | 内部 ID | UUIDv7 + 类型前缀 |
| O-02 | Schema 标准 | JSON Schema 2020-12 |
| O-03 | Python Schema 框架 | Pydantic v2 |
| O-04 | API 文档 | OpenAPI 3.1 |
| O-05 | 定义对象模式 | `metadata/spec/status` |
| O-06 | Event 格式 | CloudEvents 兼容信封 + 平台字段 |
| O-07 | 权限模型 | RBAC + ABAC + Capability，Deny 优先 |
| O-08 | Skill Bundle 摘要 | SHA-256 |
| O-09 | 定义对象版本 | Semantic Versioning |
| O-10 | Task 与 Tool 幂等 | 作用域化 Idempotency Key |
| O-11 | Worker 并发控制 | Lease + Heartbeat + Fencing Token |
| O-12 | 价格核算 | 带版本的 Model Price Snapshot |
...
冻结版本：
B0 Agent 底层对象与协议详细设计 v0.1.0

是否允许进入 JSON Schema 设计：是 / 否
```
