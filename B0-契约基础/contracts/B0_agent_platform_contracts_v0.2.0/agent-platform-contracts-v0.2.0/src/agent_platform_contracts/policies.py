...
def scoped_task_idempotency_key(tenant_id: str, source_type: str, key: str) -> str:
    return canonical_sha256({"tenant_id": tenant_id, "source_type": source_type, "idempotency_key": key})


def scoped_tool_idempotency_key(tool_id: str, target_resource: str, key: str) -> str:
    return canonical_sha256({"tool_id": tool_id, "target_resource": target_resource, "idempotency_key": key})


def can_auto_retry(*, retryable: bool, safe_to_retry: bool, tool_allows_retry: bool, state: str) -> bool:
    return retryable and safe_to_retry and tool_allows_retry and state != "unknown"


def lease_allows_commit(*, fencing_token: int, current_fencing_token: int, expires_at: datetime, now: datetime | None = None) -> bool:
