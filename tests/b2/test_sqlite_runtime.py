        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            run(
                service.validate_task(
                    task["metadata"]["id"], {"actor_type": "system", "actor_id": "test"}
                )
            )
            ...
            for event in events:
                ...
                self.assertEqual(
                    event["spec"]["schema_ref"]["digest"], canonical_sha256(event["spec"]["data"])
                )
            store.close()

            reopened = SQLiteRuntimeStore(db)
            self.assertIsNotNone(
                run(SQLiteTaskRepository(store=reopened).get_task(task["metadata"]["id"]))
            )
            self.assertEqual(
                len(run(SQLiteEventRepository(store=reopened).get_events(task["metadata"]["id"]))),
                2,
            )
            reopened.close()
