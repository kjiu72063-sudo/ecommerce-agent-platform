...
from registry import InMemoryDefinitionRepository, RegistryService, DefinitionFilter
...
class TestInMemoryRepository(unittest.TestCase):
    ...
    def test_update_with_revision(self):
        """测试乐观并发更新"""
        obj = {"metadata": {"id": "test_001", "revision": 1}, "kind": "Test", "status": {"phase": "draft"}}
        run_async(self.repo.create(obj))
    ...
    def test_list_by_filter(self):
        """测试按过滤器查询"""
        obj1 = {"metadata": {"id": "agt_001", "namespace": "dev", "key": "agent-1", "created_at": "2026-09-01T00:00:00Z"}, "kind": "AgentSpec", "status": {"phase": "active"}}
        obj2 = {"metadata": {"id": "skl_001", "namespace": "dev", "key": "skill-1", "created_at": "2026-09-01T00:00:00Z"}, "kind": "SkillManifest", "status": {"phase": "draft"}}
...
class TestRegistryService(unittest.TestCase):
    ...
    def test_register_definition(self):
        ...
        result = run_async(self.service.register_definition(
            kind="agent-spec",
            payload=payload,
    ...
    def test_register_duplicate_fails(self):
        ...
        run_async(self.service.register_definition(
            kind="agent-spec",
            payload=payload,
            actor={"actor_type": "user", "actor_id": "usr_test"}
        ))
    ...
    def test_transition_state(self):
        ...
        # draft -> testing
        result = run_async(self.service.transition_state(
            obj_id, "testing", reason="开始测试"
        ))
        self.assertEqual(result["phase"], "testing")
        
        # testing -> awaiting_approval
        result = run_async(self.service.transition_state(
            obj_id, "awaiting_approval", reason="测试通过"
        ))
    ...
    def test_invalid_transition_fails(self):
        ...
        # active -> draft（不允许）
        with self.assertRaises(ValueError):
            run_async(self.service.transition_state(
                obj_id, "draft", reason="非法迁移"
            ))
